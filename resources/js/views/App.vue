    ...
    <div>
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark sticky-top">
            <transition name="fade" mode="out-in" v-on:after-enter="initBS()">
                <component v-if="header.component && (header.force || header.visible)" :is="header.component"
                    :headerData="header.data"></component>
                <div v-else class="container">
                    ...
                    <div class="navbar-nav collapse navbar-collapse flex-grow-0 order-3 order-lg-2" id="navbar">
                        ...
                        <router-link v-if="$site.features.includes('groups')" :to="{ name: 'groups' }"
                            class="nav-item nav-link"><users-icon /> {{ $t("app.groups") }}</router-link>
                        <router-link v-if="$site.features.includes('parodies')" :to="{ name: 'parodies' }"
                            class="nav-item nav-link"><copy-icon /> {{ $t("app.parodies") }}</router-link>
                        <router-link v-if="$site.features.includes('characters')" :to="{ name: 'characters' }"
                            class="nav-item nav-link"><star-icon /> {{ $t("app.characters") }}</router-link>
                        <router-link v-if="$site.features.includes('artists')" :to="{ name: 'artists' }"
                    ...
                    <div class="navbar-nav flex-row order-2 order-lg-3">
                        <a href="#navbar" class="nav-item nav-link d-lg-none" data-toggle="collapse"
                            data-target="#navbar"><menu-icon /></a>
                        <a href="#search" class="nav-item nav-link"
                            @click.prevent="openModal('Search')"><search-icon /></a>
                        <template v-if="!isLoggedIn">
                            <router-link :to="{ name: 'login' }" class="nav-item btn btn-link" :event="''"
                                @click.native.prevent="openModal('Login')">{{ $t("auth.login") }}</router-link>
                            <router-link :to="{ name: 'register' }" class="nav-item btn btn-primary" :event="''"
                                @click.native.prevent="openModal('Register')">{{ $t("auth.register") }}</router-link>
                        </template>
                        <template v-else>
                            <div class="nav-item dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <router-link :to="{ name: 'profile', params: { username: user.username } }"
                                        class="dropdown-item profile"><img :src="user.avatar_url"> {{ user.username
                                        }}</router-link>
                                    <router-link :to="{ name: 'settings' }" class="dropdown-item"><settings-icon /> {{
                                        $t("auth.settings") }}</router-link>
                                    <template v-if="user.role == 'admin'">
                                        <div class="dropdown-divider"></div>
                                        <h6 class="dropdown-header">{{ $t("auth.administration") }}</h6>
                                        <router-link :to="{ name: 'admin/comics' }" class="dropdown-item"><grid-icon />
                                            {{ $t("admin.comics") }}</router-link>
                                        <router-link :to="{ name: 'admin/ads' }" class="dropdown-item"><layers-icon />
                                            Ad management</router-link>
                        ...
                        <div v-if="$site.features.includes('nsfw')"
                            class="nsfw-toggle custom-control custom-switch align-self-center">
        ...
        <footer class="bg-dark py-3" v-show="footer.show">
            <div class="d-flex container justify-content-between align-items-center">
                <div class="dropup">
                    <button class="btn btn-default dropdown-toggle" data-toggle="dropdown"><globe-icon /> {{
                        languages[this.$i18n.locale].name }}</button>
                ...
                <div class="d-flex align-items-center">
                    <router-link v-if="$site.features.includes('dmca')" class="mr-3"
                        :to="{ name: 'dmca' }">DMCA</router-link>
                    <router-link v-if="$site.features.includes('dmca')" :to="{ name: 'USC2257' }">18 U.S.C.
                        2257</router-link>
                    <div class="social-links">
                        <a v-for="(link, network) in networks" :key="network" :href="link" target="_blank"
                            class="float-right">

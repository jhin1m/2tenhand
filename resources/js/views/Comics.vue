    ...
    <div class="container">
        <div class="box filters d-flex justify-content-between">
            ...
            <div>
                ...
                <div class="filter">
                    <button class="btn btn-default" data-toggle="collapse" data-target="#filter"><filter-icon /> {{
                        $t('app.filters.filters') }}</button>
                </div>
            </div>
        </div>
            ...
            <div class="box filters d-flex flex-column mt-3">
                ...
                <div class="row m-0">
                    <div class="col-md-3" v-if="$site.features.includes('categories')">
                        <multiselect v-model="categoriesModel" label="name" track-by="id"
                            :placeholder="$t('app.filters.categories') | capitalize" :options="results.categories"
                            :multiple="true" :searchable="true" :loading="status.categories == 'loading'"
                            :internal-search="false" :clear-on-select="false" :close-on-select="false" :limit="2"
                            :limit-text="(count) => getMessage('limit-text', 'categories', { count })" :max-height="300"
                            :show-no-results="status.categories == 'none'" :hide-selected="true"
                            @search-change="(query) => search('categories', query)">
                    ...
                    <div class="col-md-3" v-if="$site.features.includes('tags')">
                        <multiselect v-model="tagsModel" label="name" track-by="id"
                            :placeholder="$t('app.filters.tags') | capitalize" :options="results.tags" :multiple="true"
                            :searchable="true" :loading="status.tags == 'loading'" :internal-search="false"
                            :clear-on-select="false" :close-on-select="false" :limit="2"
                            :limit-text="(count) => getMessage('limit-text', 'tags', { count })" :max-height="300"
                            :show-no-results="status.tags == 'none'" :hide-selected="true"
                            @search-change="(query) => search('tags', query)">
                    ...
                    <div class="col-md-3" v-if="$site.features.includes('artists')">
                        <multiselect v-model="artistsModel" label="name" track-by="id"
                            :placeholder="$t('app.filters.artists') | capitalize" :options="results.artists"
                            :multiple="true" :searchable="true" :loading="status.artists == 'loading'"
                            :internal-search="false" :clear-on-select="false" :close-on-select="false" :limit="2"
                            :limit-text="(count) => getMessage('limit-text', 'artists', { count })" :max-height="300"
                            :show-no-results="status.artists == 'none'" :hide-selected="true"
                            @search-change="(query) => search('artists', query)">
                            <template slot="option" slot-scope="props">
                                <img class="option__image" :src="props.option.icon_url">
                                <span>{{ props.option.name }}</span>
                            </template>
                    ...
                    <div class="col-md-3" v-if="$site.features.includes('authors')">
                        <multiselect v-model="authorsModel" label="name" track-by="id"
                            :placeholder="$t('app.filters.authors') | capitalize" :options="results.authors"
                            :multiple="true" :searchable="true" :loading="status.authors == 'loading'"
                            :internal-search="false" :clear-on-select="false" :close-on-select="false" :limit="2"
                            :limit-text="(count) => getMessage('limit-text', 'authors', { count })" :max-height="300"
                            :show-no-results="status.authors == 'none'" :hide-selected="true"
                            @search-change="(query) => search('authors', query)">
                            <template slot="option" slot-scope="props">
                                <img class="option__image" :src="props.option.icon_url">
                                <span>{{ props.option.name }}</span>
                            </template>
                    ...
                    <div class="col-md-3" v-if="$site.features.includes('groups')">
                        <multiselect v-model="groupsModel" label="name" track-by="id"
                            :placeholder="$t('app.filters.groups') | capitalize" :options="results.groups"
                            :multiple="true" :searchable="true" :loading="status.groups == 'loading'"
                            :internal-search="false" :clear-on-select="false" :close-on-select="false" :limit="2"
                            :limit-text="(count) => getMessage('limit-text', 'groups', { count })" :max-height="300"
                            :show-no-results="status.groups == 'none'" :hide-selected="true"
                            @search-change="(query) => search('groups', query)">
                            <template slot="option" slot-scope="props">
                                <img class="option__image" :src="props.option.icon_url">
                                <span>{{ props.option.name }}</span>
                            </template>
                    ...
                    <div class="col-md-3" v-if="$site.features.includes('characters')">
                        <multiselect v-model="charactersModel" label="name" track-by="id"
                            :placeholder="$t('app.filters.characters') | capitalize" :options="results.characters"
                            :multiple="true" :searchable="true" :loading="status.characters == 'loading'"
                            :internal-search="false" :clear-on-select="false" :close-on-select="false" :limit="2"
                            :limit-text="(count) => getMessage('limit-text', 'characters', { count })" :max-height="300"
                            :show-no-results="status.characters == 'none'" :hide-selected="true"
                            @search-change="(query) => search('characters', query)">
                            <template slot="option" slot-scope="props">
                                <img class="option__image" :src="props.option.icon_url">
                                <span>{{ props.option.name }}</span>
                            </template>
                    ...
                    <div class="col-md-3" v-if="$site.features.includes('parodies')">
                        <multiselect v-model="parodiesModel" label="name" track-by="id"
                            :placeholder="$t('app.filters.parodies') | capitalize" :options="results.parodies"
                            :multiple="true" :searchable="true" :loading="status.parodies == 'loading'"
                            :internal-search="false" :clear-on-select="false" :close-on-select="false" :limit="2"
                            :limit-text="(count) => getMessage('limit-text', 'parodies', { count })" :max-height="300"
                            :show-no-results="status.parodies == 'none'" :hide-selected="true"
                            @search-change="(query) => search('parodies', query)">
                            <template slot="option" slot-scope="props">
                                <img class="option__image" :src="props.option.icon_url">
                                <span>{{ props.option.name }}</span>
                            </template>
                    ...
                    <div class="col-md-3" v-if="$site.features.includes('languages')">
                        <multiselect v-model="languagesModel" label="name" track-by="id"
                            :placeholder="$t('app.filters.languages') | capitalize" :options="results.languages"
                            :multiple="true" :searchable="true" :loading="status.languages == 'loading'"
                            :internal-search="false" :clear-on-select="false" :close-on-select="false" :limit="2"
                            :limit-text="(count) => getMessage('limit-text', 'languages', { count })" :max-height="300"
                            :show-no-results="status.languages == 'none'" :hide-selected="true"
                            @search-change="(query) => search('languages', query)">
                            <template slot="option" slot-scope="props">
                                <img class="option__image" :src="props.option.icon_url">
                                <span>{{ props.option.name }}</span>
                            </template>
                    ...
                    <div class="col-md-3">
                        <multiselect v-model="filters.attributes" :options="attributes" :multiple="true"
                            :searchable="false" :placeholder="$t('app.filters.attributes') | capitalize">
                            <span class="multiselect__single" v-if="values.length > 1 && !isOpen" slot="selection"
                                slot-scope="{ values, search, isOpen }">{{ getMessage('selected', 'attributes', {
                                    count:
                                        values.length
                                }) }}</span>
                    ...
                    <div class="col-md-3" v-if="$site.features.includes('statuses')">
                        <multiselect v-model="filters.statuses" :options="statuses" :multiple="true" :searchable="false"
                            :placeholder="$t('app.filters.statuses') | capitalize">
                            <span class="multiselect__single" v-if="values.length > 1 && !isOpen" slot="selection"
                                slot-scope="{ values, search, isOpen }">{{ getMessage('selected', 'statuses', {
                                    count:
                                        values.length
                                }) }}</span>

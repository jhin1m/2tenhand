<template>
    <div :class="(modal ? '' : 'col-md-4 p-0 my-5 mx-auto ') + 'card card-default'">
        <div class="card-header text-center">
            ...
            <h6 class="card-subtitle mb-2 text-muted">{{ subtitle || $t('auth.register_text', { name: $site.name }) }}
            </h6>
        </div>
        <div class="card-body">
            <form @keyup.enter="submitting ? null : register()">
                ...
                <div class="text-center">
                    <button type="button" class="btn mt-4 btn-primary btn-lg btn-block" @click.prevent="register"
                        :disabled="submitting || registered"><span
                            class="spinner-border spinner-border-sm align-baseline" v-if="submitting"></span><template
                            v-else>{{ $t('auth.register') }}</template></button>
                </div>
            </form>
        </div>
    </div>

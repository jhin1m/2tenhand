<template>
    <div :class="(modal ? '' : 'col-md-4 p-0 my-5 mx-auto ') + 'card card-default'">
        <div class="card-header text-center">
            ...
            <h6 class="card-subtitle mb-2 text-muted">{{ $t('auth.login_text', { name: $site.name }) }}</h6>
        </div>
        <div class="card-body">
            <form @keyup.enter="submitting ? null : login()">
                ...
                <div class="d-flex">
                    ...
                    <div class="col-md-6 pr-0 text-right">
                        <router-link :to="{ name: 'password-reset' }" :event="''"
                            @click.native.prevent="openModal('PasswordReset')">{{ $t("auth.reset_password")
                            }}</router-link>
                    </div>
                </div>
                <div class="text-center">
                    <button type="button" class="btn mt-4 btn-primary btn-lg btn-block" @click.prevent="login"
                        :disabled="submitting"><span class="spinner-border spinner-border-sm align-baseline"
                            v-if="submitting"></span><template v-else>{{ $t('auth.login') }}</template></button>
                </div>
            </form>
        </div>
    </div>

        ...
        <div v-if="status == 'done'" id="popular-comics-slider" class="box clearfix d-flex flex-column w-100 mb-3">
            <div class="box-title mx-1">
                <h6 class="float-left font-weight-bolder my-1">
                    ...
                    <router-link :to="{ name: 'comics', query: { sort: 'popularity', duration } }"
                        class="ml-2 text-muted">{{ $t('app.view_all') }} <arrow-right-icon /></router-link>
                </h6>
                ...
                <div class="float-right dropdown">
                    <button class="btn btn-link py-0 dropdown-toggle" data-toggle="dropdown"><calendar-icon /> {{
                        $t('app.filters.durations.' + duration) }}</button>
                    <div class="dropdown-menu dropdown-menu-right">
                        <template v-for="option in durations">
                            <a class="dropdown-item" @click="duration = option" :key="option"
                                v-bind:class="{ active: duration == option }">{{ $t('app.filters.durations.' + option)
                                }}</a>
                        </template>
                    </div>
                </div>

<template>
    <router-link :to="{ name: 'comic', params: { slug: comic.slug } }">
        <div class="comic box clearfix d-flex flex-column h-100"
            :class="{ 'no-hover': noHover, lighter, editable: user.role === 'admin', premium: comic.premium }">
            <div class="image" :style="'background-image: url(' + comic.thumb_url + ');'" role="img"
                :aria-label="`${comic.title} cover`" :alt="comic.title"></div>

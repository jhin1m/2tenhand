<template>
    <div>
        <div v-if="status.recommendations == 'loading'" class="box text-center py-5 w-100">
            <div class="spinner-border"></div>
        </div>
        <div id="recommendations-slider" class="box clearfix d-flex flex-column w-100"
            v-else-if="recommendations.length && this.status.recommendations != 'error'">
            <h6 class="box-header">
                    ...
                    <ul class="glide__slides py-1" style="padding: 10px 0px;">
                        <li class="glide__slide" v-for="(recommendation, index) in recommendations" :key="index">
                            <slot :recommendation="recommendation">
                                <comic :comic="recommendation" :lighter="true" />
                            </slot>
                        </li>
                    </ul>
                </div>
